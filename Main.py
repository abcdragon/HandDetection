...
while not startTrack:
    ...
    if contours:
        ...
        #Contour는 도형의 윤곽선(외곽선)
        #Convex Hull 은 도형의 꼭짓점을 이은 선분
        ...
        if defects is not None:
            ...
            for i in range(defects.shape[0]):
                s, e, f, d = defects[i, 0]
                start = tuple(cnt[s][0]) # 시작점
                end = tuple(cnt[e][0]) # 연결점
                far = tuple(cnt[f][0]) # contour에서 가장 먼 점

                cv2.circle(ROI, start, 8, [255, 0, 0], -1)
                cv2.circle(ROI, end  , 8, [0, 255, 0], -1)
                ...
                if angle <= math.pi / 2:  # angle less than 90 degree, treat as fingers -> 90도 아래면 손가락
                    fingers += 1
                    cv2.circle(ROI, far, 1, [200, 200, 200], -1)
                '''
                if len(cnt) >= 5:
